...
source: docs/specs/ordercore-techspec.pdf
status: draft
---
...
| --------- | ----------------------------------- | -------------------------------------- | ----------------------------------------- |
| OrderCore | python -m pytest apps/ordercore/tests | Validation, lifecycle, the fallback path | Anything that reaches Billing over the wire |
| Billing   | JUnit, no runner installed          | Calculation, request parsing           | What OrderCore actually sends             |
| Lint      | python -m ruff check apps/ordercore | Style, unused imports                  | Correctness                               |

## 7. Known gaps

| Ref  | Gap                                    | Consequence                              | Owner     |
| ---- | -------------------------------------- | ---------------------------------------- | --------- |
| TG-1 | No contract test across the wire       | A field can be dropped in transit unnoticed | Architect |
| TG-2 | Tax rate duplicated in two languages   | Silent divergence                        | Architect |
| TG-3 | Invoice p95 over the NFR since February | Undetected breach                        | Developer |
